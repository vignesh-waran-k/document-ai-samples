...
def get_page_text_anc_mentiontext(
    ...
    method, matches, match_string_pair = "", [], []

    # Attempt to find matches with provided texts
    for text in [entity.mention_text, mapping_text]:
        matches, match_string_pair = find_substring_indexes(orig_text, text)
        if matches:
            method = "OCR-EntityMT" if text == entity.mention_text else "OCR-TU"
            break

    # If no matches, attempt date matching
    if not matches:
        matched_dates = match_dates(orig_text, entity.mention_text)
        for date_matches in matched_dates.values():
            for mat_11 in date_matches:
                match_temp, match_string_pair_temp = find_substring_indexes(orig_text, mat_11)
                matches.extend(match_temp)
                match_string_pair.extend(match_string_pair_temp)
            if matches:
                method = "OCR-EntityMT"
                break
    ...
    for match, str_pair in zip(matches, match_string_pair):
        try:
            text_segments = [documentai.Document.TextAnchor.TextSegment(start_index=int(match[0]), end_index=int(match[1]))]
            bb, text_anc = get_token(orig_invoice_json, english_page_num, text_segments)
            if not bb:
                continue
            
            # Check bbox proximity
            if abs(bb["min_y"] - min_y) <= diff_y and abs(bb["min_x"] - min_x) <= diff_x:
                bbox, text_anc_1 = bb, text_anc
                new_mention_text = construct_new_mention_text(orig_text, text_anc_1)
