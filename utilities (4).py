...
def get_f1_score(
    project_id: str, location: str, processor_id: str, processor_version: str
) -> float:
    """It give over all F1 Score of a processor for provided processor version

    Args:
        project_id (str): GCP Project Id
        location (str): Processor Location either "us" or "eu"
        processor_id (str): DocumentAi Processor Id
        processor_version (str): Provide target processor id of as processor

    Returns:
        float: Returns over all f1 Score of a processor for provided processor version
    """

    opt = {"api_endpoint": f"{location}-documentai.googleapis.com"}
    client = documentai.DocumentProcessorServiceClient(client_options=opt)
    name = client.processor_version_path(
        project=project_id,
        location=location,
    ...
    request = documentai.ListEvaluationsRequest(parent=name)

    # Taking the latest evaluation by default
    page_result = client.list_evaluations(request=request)
    name = ""
    for i in page_result.pages:
        for j in i.evaluations:
            name = j.name
            break

    request = documentai.GetEvaluationRequest(name=name)
    eval_result = client.get_evaluation(request)

    # get f1 score for all samples for each entity
    entity_dict1 = defaultdict(list)
    for key, value in eval_result.entity_metrics.items():
        for clm in value.confidence_level_metrics:
            entity_dict1[key].append(clm.metrics.f1_score)

    # get maximum f1 score for all samples for each entity
    final = {}
    for key in entity_dict1:
        final[key] = max(entity_dict1[key])

    parent_items = []
    for key in final:
        if "/" in str(key):
            parent_items.append(key.split("/")[0])
    parent_items = list(set(parent_items))

    # get metrics-proto for all entities
    final_dict = {}
    for key, value in eval_result.entity_metrics.items():
        if key in parent_items:
            continue
        for clm in value.confidence_level_metrics:
            if clm.metrics.f1_score == final[key]:
                final_dict[key] = clm.metrics
    ...
    # Calculate False Positives, True Positives & False Negatives
    total_false_positive = 0
    total_true_positive = 0
    total_false_negative = 0

    for key, value in final_dict.items():
        total_false_positive = total_false_positive + value.false_positives_count
        total_true_positive = total_true_positive + value.true_positives_count
        total_false_negative = total_false_negative + value.false_negatives_count
    ...
    overall_f1_score = 2 * (recall * precision) / (recall + precision)

    print(f"Processor Id - {processor_id}")
    print(f"Processor Version - {processor_version}")
